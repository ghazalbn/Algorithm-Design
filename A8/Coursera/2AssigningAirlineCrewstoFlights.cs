...
namespace _8_2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var flightCount = line[0];
            var crewCount = line[1];
            var info = new int[flightCount][];
            for (int i = 0; i < flightCount; i++)
                info[i] = Array.ConvertAll(Console.ReadLine().Split(), int.Parse);
        ...
        private static void BuildGraph(long N, long M, int[][] info)
        {
            // source to v
            for (long i = 0; i < N; i++)
            {
        ...
        public static long[] EdmondKarp(long N, long nodeCount, long s, long t) 
        { 
            rGraph = graph;
